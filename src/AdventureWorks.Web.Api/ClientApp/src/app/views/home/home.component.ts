...
export class HomeComponent implements OnInit {
  ...
  constructor(private readonly dynamicThemeService: DynamicThemeService, private readonly toastMessageService: ToastMessageService, private readonly authService: AuthService, private readonly componentCacheService: ComponentCacheService, public dialog: MatDialog) {}

  ngOnInit(): void {
    this.authService.userProfile$.pipe(takeUntil(this._destroy$)).subscribe((result) => {
  ...
  showToastMessage() {
    this.toastMessageService.show('Unable to load the list of security policies. Please try again or reload the page.', environment.shortToastMessageDuration);
  }

  showToastAlert() {
    this.toastMessageService.show('Unable to load the list of items. Please try again or reload the page.', environment.longToastMessageDuration, true, 'Reload', () => {
