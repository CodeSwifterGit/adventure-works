...
export class DataService {
  ...
  httpOptions(parameters?: HttpParams, observe: any = 'body', reportProgress: boolean = false, timeout: number = environment.waitForPendingRemoteDataRequest) {
    const headers = new HttpHeaders().append('Content-Type', 'application/json').append(environment.requestTimeoutHeaderName, timeout.toString());

    if (!parameters) parameters = new HttpParams();
