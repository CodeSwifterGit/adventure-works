...
{
    public partial class UpdateCustomerCommandHandler : IRequestHandler<UpdateCustomerCommand, CustomerLookupModel>
    {
        ...
        public UpdateCustomerCommandHandler(IAdventureWorksContext context,
        ...
        public async Task<CustomerLookupModel> Handle(UpdateCustomerCommand request, CancellationToken cancellationToken)
        {
            var entity = await _context.Customers
                .SingleAsync(c => c.CustomerID == request.RequestTarget.CustomerID, cancellationToken);
            var oldEntity = _mapper.Map<Entities.Customer, UpdateCustomerCommand>(entity);

            if (entity == null)
            {
                throw new NotFoundException(nameof(Customer), JsonConvert.SerializeObject(new { request.RequestTarget.CustomerID }).Replace("{", "[").Replace("}", "]"));
            }
            ...
            entity = await _context.Customers.SingleAsync(c => c.CustomerID == request.RequestTarget.CustomerID, cancellationToken);
