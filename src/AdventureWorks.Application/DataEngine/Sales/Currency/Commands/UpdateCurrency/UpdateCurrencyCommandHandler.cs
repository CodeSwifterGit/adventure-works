...
{
    public partial class UpdateCurrencyCommandHandler : IRequestHandler<UpdateCurrencyCommand, CurrencyLookupModel>
    {
        ...
        public UpdateCurrencyCommandHandler(IAdventureWorksContext context,
        ...
        public async Task<CurrencyLookupModel> Handle(UpdateCurrencyCommand request, CancellationToken cancellationToken)
        {
            var entity = await _context.Currencies
                .SingleAsync(c => c.CurrencyCode == request.RequestTarget.CurrencyCode, cancellationToken);
            var oldEntity = _mapper.Map<Entities.Currency, UpdateCurrencyCommand>(entity);

            if (entity == null)
            {
                throw new NotFoundException(nameof(Currency), JsonConvert.SerializeObject(new { request.RequestTarget.CurrencyCode }).Replace("{", "[").Replace("}", "]"));
            }
            ...
            entity = await _context.Currencies.SingleAsync(c => c.CurrencyCode == request.RequestTarget.CurrencyCode, cancellationToken);

            await _mediator.Publish(new CurrencyUpdated()
            {
                Entity = entity
            }, cancellationToken);
