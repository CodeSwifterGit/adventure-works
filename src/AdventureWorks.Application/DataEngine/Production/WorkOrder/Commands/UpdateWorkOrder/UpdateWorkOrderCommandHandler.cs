...
{
    public partial class UpdateWorkOrderCommandHandler : IRequestHandler<UpdateWorkOrderCommand, WorkOrderLookupModel>
    {
        ...
        public UpdateWorkOrderCommandHandler(IAdventureWorksContext context,
        ...
        public async Task<WorkOrderLookupModel> Handle(UpdateWorkOrderCommand request, CancellationToken cancellationToken)
        {
            var entity = await _context.WorkOrders
                .SingleAsync(c => c.WorkOrderID == request.RequestTarget.WorkOrderID, cancellationToken);
            var oldEntity = _mapper.Map<Entities.WorkOrder, UpdateWorkOrderCommand>(entity);

            if (entity == null)
            {
                throw new NotFoundException(nameof(WorkOrder), JsonConvert.SerializeObject(new { request.RequestTarget.WorkOrderID }).Replace("{", "[").Replace("}", "]"));
            }
            ...
            entity = await _context.WorkOrders.SingleAsync(c => c.WorkOrderID == request.RequestTarget.WorkOrderID, cancellationToken);

            await _mediator.Publish(new WorkOrderUpdated()
            {
                Entity = entity
            }, cancellationToken);
