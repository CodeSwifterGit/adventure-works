...
namespace AdventureWorks.Persistence.Infrastructure
{
    public abstract class DesignTimeDbContextFactoryBase<TContext> :
        IDesignTimeDbContextFactory<TContext> where TContext : DbContext
    {
        private const string ConnectionStringName = "AdventureWorksDatabase";
        private const string AspNetCoreEnvironment = "DOTNET_ENVIRONMENT";

        public TContext CreateDbContext(string[] args)
        {
            var basePath = Directory.GetCurrentDirectory() +
                           string.Format("{0}..{0}AdventureWorks.Web.Api", Path.DirectorySeparatorChar);
