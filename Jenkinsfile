pipeline {
  ...
  environment {
    ...
    DOCKER_REPOSITORY_SECRET='adventureworks-docker-hub'
    APP_IMAGE_NAME='change_me/adventureworks-webapp'
    TEST_IMAGE_NAME='change_me/adventureworks-test'
    // Make sure to have kubectl installed and accessible through $PATH variable
    KUBECTL_CONFIG_SECRET=credentials('do-adventureworks-kube-config')
        ...
        script {
          sh label:
            ...
            script: '''
              ...
              cd $WORKSPACE/src/AdventureWorks.Web.Api/
              docker images -a
              docker build -f "$WORKSPACE/src/AdventureWorks.Application.Tests/Dockerfile" --force-rm -t ${TEST_IMAGE_NAME} "$WORKSPACE"
              docker images -a
            '''
        }
      }
      ...
      steps {
        sh label:
          ...
          script: '''
            ...
            cd $WORKSPACE/src/AdventureWorks.Web.Api/
            docker images -a
            docker build -f "$WORKSPACE/src/AdventureWorks.Web.Api/Dockerfile"  --build-arg BUILD_NUMBER=${BUILD_NUMBER} --build-arg GIT_COMMIT=${GIT_COMMIT} --build-arg BUILD_CONFIGURATION=${BUILD_CONFIGURATION} --force-rm -t ${APP_IMAGE_NAME}:${BRANCH_NAME}-${BASE_VERSION}.${BUILD_NUMBER} "$WORKSPACE"
            docker images -a
          '''
      }
      post {
        ...
        script {
          sh label:
            ...
            script: '''
               ...
               docker rmi $(docker images -f "reference=change_me/adventureworks*" -q)
            '''
        }
      }
      post {
            ...
            steps {
              script {
                sh label:
                  ...
                  script: '''
                    kubectl --kubeconfig "${KUBECTL_CONFIG_SECRET}" scale deployment adventureworks-app-staging --replicas=0
                '''
              }
            }
            post {
      ...
      steps {
        script {
          sh label:
            ...
            script: '''
              kubectl --kubeconfig "${KUBECTL_CONFIG_SECRET}" patch deployment adventureworks-app-staging -p \"{\\"spec\\":{\\"template\\":{\\"metadata\\":{\\"annotations\\":{\\"version\\":\\"${BASE_VERSION}.${BUILD_NUMBER}\\"}},\\"spec\\":{\\"containers\\":[{\\"name\\":\\"adventureworks-webapp\\",\\"image\\":\\"${APP_IMAGE_NAME}:${BRANCH_NAME}-${BASE_VERSION}.${BUILD_NUMBER}\\"}]}}}}\"
              kubectl --kubeconfig "${KUBECTL_CONFIG_SECRET}" scale deployment adventureworks-app-staging --replicas=1
          '''
        }
      }
      post {
        success {
          echo 'AdventureWorks deployed successfully to staging environment'
        }
        failure {
          error 'Error deploying AdventureWorks to staging environment'
        }
      }
    }
      ...
      steps {
        script {
          sh label:
            ...
            script: '''
              kubectl --kubeconfig "${KUBECTL_CONFIG_SECRET}" patch deployment adventureworks-app-production -p \"{\\"spec\\":{\\"template\\":{\\"metadata\\":{\\"annotations\\":{\\"version\\":\\"${BASE_VERSION}.${BUILD_NUMBER}\\"}},\\"spec\\":{\\"containers\\":[{\\"name\\":\\"adventureworks-webapp\\",\\"image\\":\\"${APP_IMAGE_NAME}:${BRANCH_NAME}-${BASE_VERSION}.${BUILD_NUMBER}\\"}]}}}}\"
          '''
        }
      }
      post {
        success {
          echo 'AdventureWorks deployed successfully to production environment'
        }
        failure {
          error 'Error deploying AdventureWorks to production environment'
        }
      }
    }
  }
